...
class Model(object):
    ...
    def __init__(self, user_count, item_count, cate_count, cate_list):
        ...
        cate_list = tf.convert_to_tensor(cate_list,
                                         dtype=tf.int64)  # convert list to tensor before use tf function on it
        ...
        mask = tf.sequence_mask(self.sl, tf.shape(h_emb)[1],
                                dtype=tf.float32)  # [B, T] Returns a mask tensor representing the first N positions of each cell.
        mask = tf.expand_dims(mask, -1)  # [B, T, 1]
        mask = tf.tile(mask, [1, 1, tf.shape(h_emb)[
            2]])  # [B, T, H] This operation creates a new tensor by replicating input multiples times
        h_emb *= mask  # [B, T, H]
        hist = h_emb
        hist = tf.reduce_sum(hist, 1)  # Computes the sum of elements across dimensions of a tensor
        hist = tf.div(hist, tf.cast(tf.tile(tf.expand_dims(self.sl, 1), [1, 128]),
                                    tf.float32))  # Divides x / y elementwise （sum average). Casts a tensor to a new type.
